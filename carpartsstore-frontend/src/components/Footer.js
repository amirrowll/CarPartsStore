...
import { Car, MapPin, Shield, Truck, Clock, CreditCard } from 'lucide-react';
...
const FooterFixed = () => {
    ...
    useEffect(() => {
        ...
        items.forEach((item, index) => {
            ...
            item.style.marginTop = `-${itemWidth / 2}px`;
        ...
        const animate = () => {
            ...
            items.forEach((item, index) => {
                ...
                item.style.zIndex = Math.round(opacity * 10).toString();
            });
    ...
    return (<footer className="bg-gray-900 text-white">
      <div className="bg-white py-4 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div ref={containerRef} className="relative overflow-hidden flex items-center justify-center" style={{ height: '160px' }}>
            {/* تعداد بیشتری از برندها برای لوپ بینهایت */}
            {[...brands, ...brands, ...brands, ...brands, ...brands, ...brands, ...brands].map((brand, index) => (<div key={`${brand.name}-${index}`} className="brand-item absolute top-1/2 left-0" style={{
                ...
                <div className="w-full h-full bg-white rounded-xl border border-gray-200 flex flex-col items-center justify-center shadow-lg" style={{ padding: '12px' }}>
                  <img src={brand.logo} alt={brand.name} className="max-h-7 max-w-full object-contain mb-2" style={{ maxHeight: '28px' }} onError={(e) => {
      ...
      <div className="bg-gray-800 py-8">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {features.map((feature, index) => (<div key={index} className="flex items-start space-x-4 space-x-reverse">
              ...
              <div className="flex items-center space-x-3 space-x-reverse mb-6">
                ...
                <div>
                  <h2 className="text-2xl font-heading font-bold">
                    <span className="text-red-600">P</span>inpart<span className="text-accent-400">Store</span>
                  </h2>
            ...
            {Object.entries(footerLinks).map(([category, links]) => (<div key={category}>
                ...
                <ul className="space-y-3">
                  {links.map((link) => (<li key={link.path}>
                      {link.path.startsWith('http') || link.path.startsWith('tel') || link.path.startsWith('mailto') ? (<a href={link.path} target={link.path.startsWith('http') ? '_blank' : '_self'} rel={link.path.startsWith('http') ? 'noopener noreferrer' : ''} className="text-gray-300 hover:text-accent-400 transition-colors duration-200 flex items-center space-x-reverse">
                          <span className="w-2 h-2 bg-accent-400 rounded-full ml-2"></span>
                          {link.name}
                        </a>) : (<Link to={link.path} className="text-gray-300 hover:text-accent-400 transition-colors duration-200 flex items-center space-x-reverse">
                          <span className="w-2 h-2 bg-accent-400 rounded-full ml-2"></span>
                          {link.name}
                        </Link>)}
                    </li>))}
